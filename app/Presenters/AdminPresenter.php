...
final class AdminPresenter extends Nette\Application\UI\Presenter
{
    ...
    protected function createComponentPortfolioItemForm(): Form
    {
        ...
        $form->addUpload('image', 'Portfolio Image (800x533px):')
            ->addCondition(Form::FILLED) // Only apply the following rules if the field is filled
        ...
        $form->addSubmit('delete', 'Delete this post')
    ...
    public function portfolioItemFormSucceeded(Form $form, Nette\Utils\ArrayHash $values): void
    {
        ...
        if ($form['delete']->isSubmittedBy()) {
            ...
            $this->handleDeletePortfolioItem($id);
        } else {
            $image = $values->image;
    ...
    public function handleDeletePortfolioItem(int $id): void
    {
        $portfolioItem = $this->database->table('portfolio')->get($id);
        if (!$portfolioItem) {
            $this->flashMessage("Portfolio item not found.", 'error');
        } else {
            $portfolioItem->delete();
            $this->flashMessage("Portfolio item has been successfully deleted.", 'success');
        }
